...
This rental system s role-handles vehicles, customers, bookings, authentication.
## Live Demo
...
### Vehicles
`POST /api/v1/vehicles`- Admin only - Add a vehicle
...
`PUT /api/v1/users/:userId`- Admin or Own - Update profile
`DELETE /api/v1/users/:userId`- Admin only - Delete user
### Bookings
`POST /api/v1/bookings`- Admin or customer - Create a rental booking
